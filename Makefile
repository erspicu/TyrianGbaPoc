...
HOTPATH_ASM ?= 1
DETAIL_EFFECT_ASM ?= 1
SPRITE2_EXACT_LOOKUP_ASM ?= 1
STARFIELD_BATCH_ASM ?= 1
PROJECTILE_CACHE_HINT ?= 1
PROJECTILE_HINT_ASM ?= 0
EFFECT_ACTIVE_MASK ?= 1
POOL_BIT_SCAN_ASM ?= 0
PLAYER_SHOT_FREE_MASK ?= 1
PLAYER_SHOT_UPDATE_MASK ?= 1
ENEMY_ACTIVE_MASK ?= 1
COLLISION_SNAPSHOT ?= 1
COLLISION_ACTIVE_DIRECTORY ?= 1
ROUTE_EPISODE ?= 1
ROUTE_SECTION ?= 5
...
STRESS_FAST_FORWARD_POSITION ?= 0
STRESS_FAST_FORWARD_TICKS ?= 8
STRESS_BOSS_WINDOW_VBLANKS ?= 0
STRESS_FIRE ?= 1
STRESS_RELEASE_RUNTIME ?= 0
STRESS_RELEASE_FRONT_ID ?= 8
STRESS_RELEASE_FRONT_POWER ?= 11
STRESS_RELEASE_REAR_ID ?= 30
STRESS_RELEASE_REAR_POWER ?= 11
STRESS_RELEASE_LEFT_SIDEKICK ?= 10
STRESS_RELEASE_RIGHT_SIDEKICK ?= 10
STRESS_RELEASE_SPECIAL ?= 6
STRESS_RELEASE_GENERATOR ?= 6
STRESS_RELEASE_SHIELD ?= 9
STRESS_RELEASE_SHIP ?= 2
...
ifneq ($(SPRITE2_EXACT_LOOKUP_ASM),0)
ifneq ($(SPRITE2_EXACT_LOOKUP_ASM),1)
$(error SPRITE2_EXACT_LOOKUP_ASM must be 0 or 1)
endif
endif

ifneq ($(STARFIELD_BATCH_ASM),0)
ifneq ($(STARFIELD_BATCH_ASM),1)
$(error STARFIELD_BATCH_ASM must be 0 or 1)
endif
endif

ifneq ($(PROJECTILE_CACHE_HINT),0)
ifneq ($(PROJECTILE_CACHE_HINT),1)
$(error PROJECTILE_CACHE_HINT must be 0 or 1)
endif
endif

ifneq ($(PROJECTILE_HINT_ASM),0)
ifneq ($(PROJECTILE_HINT_ASM),1)
$(error PROJECTILE_HINT_ASM must be 0 or 1)
endif
endif

ifneq ($(EFFECT_ACTIVE_MASK),0)
ifneq ($(EFFECT_ACTIVE_MASK),1)
$(error EFFECT_ACTIVE_MASK must be 0 or 1)
endif
endif

ifneq ($(POOL_BIT_SCAN_ASM),0)
ifneq ($(POOL_BIT_SCAN_ASM),1)
$(error POOL_BIT_SCAN_ASM must be 0 or 1)
endif
endif

ifneq ($(PLAYER_SHOT_FREE_MASK),0)
ifneq ($(PLAYER_SHOT_FREE_MASK),1)
$(error PLAYER_SHOT_FREE_MASK must be 0 or 1)
endif
endif

ifneq ($(PLAYER_SHOT_UPDATE_MASK),0)
ifneq ($(PLAYER_SHOT_UPDATE_MASK),1)
$(error PLAYER_SHOT_UPDATE_MASK must be 0 or 1)
endif
endif

ifneq ($(ENEMY_ACTIVE_MASK),0)
ifneq ($(ENEMY_ACTIVE_MASK),1)
$(error ENEMY_ACTIVE_MASK must be 0 or 1)
endif
endif

ifneq ($(COLLISION_SNAPSHOT),0)
ifneq ($(COLLISION_SNAPSHOT),1)
$(error COLLISION_SNAPSHOT must be 0 or 1)
endif
endif

ifneq ($(COLLISION_ACTIVE_DIRECTORY),0)
ifneq ($(COLLISION_ACTIVE_DIRECTORY),1)
$(error COLLISION_ACTIVE_DIRECTORY must be 0 or 1)
endif
endif
...
ifeq ($(STRESS_RELEASE_RUNTIME),0)
STRESS_RUNTIME_TAG :=
STRESS_RUNTIME_FLAGS := -DTYRIAN_GBA_STRESS_LOADOUT=1
else ifeq ($(STRESS_RELEASE_RUNTIME),1)
STRESS_RUNTIME_TAG := _release
STRESS_RUNTIME_FLAGS := \
	-DTYRIAN_GBA_STRESS_LOADOUT=0 \
	-DAUTOTEST_RELEASE_LOADOUT=1 \
	-DAUTOTEST_RELEASE_FRONT_ID=$(STRESS_RELEASE_FRONT_ID) \
	-DAUTOTEST_RELEASE_FRONT_POWER=$(STRESS_RELEASE_FRONT_POWER) \
	-DAUTOTEST_RELEASE_REAR_ID=$(STRESS_RELEASE_REAR_ID) \
	-DAUTOTEST_RELEASE_REAR_POWER=$(STRESS_RELEASE_REAR_POWER) \
	-DAUTOTEST_RELEASE_LEFT_SIDEKICK=$(STRESS_RELEASE_LEFT_SIDEKICK) \
	-DAUTOTEST_RELEASE_RIGHT_SIDEKICK=$(STRESS_RELEASE_RIGHT_SIDEKICK) \
	-DAUTOTEST_RELEASE_SPECIAL=$(STRESS_RELEASE_SPECIAL) \
	-DAUTOTEST_RELEASE_GENERATOR=$(STRESS_RELEASE_GENERATOR) \
	-DAUTOTEST_RELEASE_SHIELD=$(STRESS_RELEASE_SHIELD) \
	-DAUTOTEST_RELEASE_SHIP=$(STRESS_RELEASE_SHIP)
else
$(error STRESS_RELEASE_RUNTIME must be 0 or 1)
endif
...
STRESS_RUN_TAG := ep$(STRESS_EPISODE)_s$(STRESS_SECTION)_$(STRESS_STOP_TAG)$(STRESS_FAST_FORWARD_TAG)$(STRESS_BOSS_TAG)$(STRESS_INPUT_TAG)$(STRESS_RUNTIME_TAG)
...
RES := res
STRESS_MAIN_OBJECT := \
	$(BUILD)/tgw8_main_$(STRESS_RUN_TAG)_$(STRESS_DIAGNOSTIC)_h$(HOTPATH_ASM)_$(CONFIG_SUFFIX)_x$(SPRITE2_EXACT_LOOKUP_ASM)_s$(STARFIELD_BATCH_ASM)_ph$(PROJECTILE_CACHE_HINT)_pa$(PROJECTILE_HINT_ASM)_fx$(EFFECT_ACTIVE_MASK)_bs$(POOL_BIT_SCAN_ASM)_sf$(PLAYER_SHOT_FREE_MASK)_su$(PLAYER_SHOT_UPDATE_MASK)_em$(ENEMY_ACTIVE_MASK)_cs$(COLLISION_SNAPSHOT)_cd$(COLLISION_ACTIVE_DIRECTORY).o
...
CFLAGS += \
	...
	-DTYRIAN_GBA_PLAYER_SHOT_UPDATE_MASK=$(PLAYER_SHOT_UPDATE_MASK) \
...
STRESS_LEVEL_OBJECT := \
	$(BUILD)/opentyrian_level_port_stress_$(STRESS_DIAGNOSTIC)_hotpath$(HOTPATH_ASM)_$(CONFIG_SUFFIX)_em$(ENEMY_ACTIVE_MASK)_cs$(COLLISION_SNAPSHOT)_cd$(COLLISION_ACTIVE_DIRECTORY).o
...
$(STRESS_MAIN_OBJECT): \
	...
	$(CC) $(CFLAGS) -DAUTOTEST \
		...
		-DAUTOTEST_FULL_LOADOUT_STRESS_FAST_FORWARD_POSITION=$(STRESS_FAST_FORWARD_POSITION) \
		-DAUTOTEST_FULL_LOADOUT_STRESS_FAST_FORWARD_TICKS=$(STRESS_FAST_FORWARD_TICKS) \
		-DAUTOTEST_FULL_LOADOUT_STRESS_BOSS_WINDOW_VBLANKS=$(STRESS_BOSS_WINDOW_VBLANKS) \
		-DAUTOTEST_STACK_CANARY \
		-DTYRIAN_GBA_DEV_PLAYER_INVINCIBLE=1 \
		$(STRESS_RUNTIME_FLAGS) \
		-DTYRIAN_GBA_SPRITE2_EXACT_LOOKUP_ASM=$(SPRITE2_EXACT_LOOKUP_ASM) \
		-DTYRIAN_GBA_STARFIELD_BATCH_ASM=$(STARFIELD_BATCH_ASM) \
		-DTYRIAN_GBA_PROJECTILE_CACHE_HINT=$(PROJECTILE_CACHE_HINT) \
		-DTYRIAN_GBA_PROJECTILE_HINT_ASM=$(PROJECTILE_HINT_ASM) \
		-DTYRIAN_GBA_EFFECT_ACTIVE_MASK=$(EFFECT_ACTIVE_MASK) \
		-DTYRIAN_GBA_POOL_BIT_SCAN_ASM=$(POOL_BIT_SCAN_ASM) \
		-DTYRIAN_GBA_PLAYER_SHOT_FREE_MASK=$(PLAYER_SHOT_FREE_MASK) \
		-DTYRIAN_GBA_PLAYER_SHOT_UPDATE_MASK=$(PLAYER_SHOT_UPDATE_MASK) \
		-DTYRIAN_GBA_ENEMY_ACTIVE_MASK=$(ENEMY_ACTIVE_MASK) \
		-DTYRIAN_GBA_COLLISION_SNAPSHOT=$(COLLISION_SNAPSHOT) \
		-DTYRIAN_GBA_COLLISION_ACTIVE_DIRECTORY=$(COLLISION_ACTIVE_DIRECTORY) \
		$(STRESS_INPUT_FLAG) \
		$(STRESS_DIAGNOSTIC_FLAGS) \
		-MMD -MP -c $< -o $@
