...
def derive_level_entries(level: Level) -> tuple[list[ManifestEntry], dict]:
    ...
    for cohort_id, cohort in enumerate(cohorts, 1):
        ...
        for event in cohort_spawns:
            previous = selected.get(event.index)
            if previous is not None and previous[1] != cohort_id:
                raise ValueError(
                    "one Boss spawn belongs to two non-merged cohorts: "
                    f"EP{level.episode} LVL{level.number} event "
                    f"{event.index}"
                )
            previous_mask = previous[0] if previous is not None else 0
            selected[event.index] = (
                previous_mask | event.spawn_mask,
                cohort_id,
            )
    ...
    entries = [
        ...
        for event_index, (spawn_mask, cohort_id) in sorted(selected.items())
    ]
...
def validate_manifest(
    ...
    for entry in entries:
        ...
        if (
            event.spawn_mask == 0
            or entry.spawn_mask != event.spawn_mask
            or entry.cohort == 0
            or entry.cohort > 0xFF
        ):
...
def write_header(path: Path, entries: list[ManifestEntry]) -> None:
    max_cohorts = max((entry.cohort for entry in entries), default=0)
    lines = [
        ...
        (
            "#define OT_BOSS_MANIFEST_MAX_COHORTS_PER_LEVEL "
            f"{max_cohorts}u"
        ),
        "",
        (
    ...
    lines.extend(
        [
            ...
            (
                "static const uint8_t "
                "ot_boss_manifest_cohorts[OT_BOSS_MANIFEST_ENTRY_COUNT] = {"
            ),
        ]
    )
    for entry in entries:
        lines.append(f"    {entry.cohort}u,")
    lines.extend(["};", "", "#endif", ""])
