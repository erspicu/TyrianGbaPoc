(() => {
  ...
  const english = Object.freeze({
    ...
    /* v89 release and LOW website refresh */
    "正式 LOW／Normal 版本，正常傷害、一般劇情裝備，包含 41 首完整 九聲道 OPL2 轉換音樂。ROM 為 27,090,252 bytes（25.835 MiB）。": "The formal LOW/Normal build uses normal damage and campaign equipment, with complete nine-channel OPL2 conversions for all 41 songs. The ROM is 27,090,252 bytes (25.835 MiB).",
    "下載 v89 LOW ROM": "Download the v89 LOW ROM",
    ...
    "v89 正式版採 LOW Detail、Normal Speed、正常傷害與一般劇情裝備， 並包含完整九聲道音訊。ROM 以 GitHub Release 資產提供，不存進 原始碼 Git；也可以直接從獨立建置環境自行產生。": "The formal v89 build uses LOW Detail, Normal Speed, normal damage, regular campaign equipment, and complete nine-channel audio. The ROM is published as a GitHub Release asset rather than stored in source Git, and can also be rebuilt from the self-contained environment.",
    ...
    "八條已開放雙向路徑各做 120 次，共 960 次。原始完整矩陣以 High／Normal 驗證，結果為 0 missed VBlank、0 runtime SHP、 0 runtime Sprite2、music active；最大單 phase 118,465 cycles，低於 180,000-cycle 回歸門檻。v89 LOW 正式版沿用相同、與 detail 無關的管線。": "Eight enabled bidirectional paths run 120 times each, for 960 transitions. The original full matrix used High/Normal and recorded zero missed VBlanks, zero runtime SHP decodes, zero runtime Sprite2 decodes, and continuous music. The largest phase cost 118,465 cycles, below the 180,000-cycle regression limit. The v89 LOW release uses the same detail-independent pipeline.",
    ...
    "對照圖保留當時 v57 的相同鏡位，方便呈現演算法前後差異；v89 LOW 正式版仍由同一套全 62 關訓練與雙感知色差 gate 產生背景 palette。": "The comparison keeps the original v57 camera position to show the algorithmic difference. The v89 LOW release still generates background palettes through the same 62-stage training set and dual perceptual-error gates.",
    "原始 Episode 2 High／Normal A/B 中，25-row 柔性鏡頭與固定 crop 都是 41 missed VBlanks；也就是增加視野活動範圍後，背景換列成本仍回到 固定鏡頭基線。v89 LOW 正式版沿用相同 camera 與 ownership band； dynamic frame drop 保存的 held window 也使用 camera-adjusted scroll，延後 presentation 時不會提前釋放仍在畫面上的 row。": "In the original Episode 2 High/Normal A/B, both the 25-row soft camera and fixed crop recorded 41 missed VBlanks. The broader viewing range therefore returns background-row cost to the fixed-camera baseline. The v89 LOW release uses the same camera and ownership band; held windows in dynamic frame drop also use camera-adjusted scroll so deferred presentation never releases rows that remain visible.",
    ...
    "Missed VBlank 分為 gameplay、stats、game over、transition 與 frontend other。v89 LOW 正式版的發佈驗證另固定檢查：": "Missed VBlanks are split among gameplay, stats, game over, transitions, and other frontend work. The v89 LOW release validation also fixes these checks:",
    ...
    "v89 LOW 正式管線中的多部件 Boss、玩家彈幕與爆炸壓力畫面": "Multipart boss, player projectiles, and explosion pressure in the formal v89 LOW pipeline",
    "最終原則": "Final rule",
