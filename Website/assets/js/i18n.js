(() => {
  ...
  const english = Object.freeze({
    ...
    "上一篇：Audio pipeline": "Previous: Audio pipeline",
    ...
    "可驗證的 payload 去重": "Verifiable payload deduplication",
    "Payload 去重": "Payload deduplication",
    "ROMFS 不以「目前沒有呼叫」為刪除條件。只有另一份 cartridge payload 已完整承接，且 build 能做無損 round-trip 時，才排除重複 原始流。目前 SND、TYREND.ANM 與 34 份": "ROMFS data is not removed merely because current code does not call it. A source stream is excluded only when another cartridge payload fully replaces it and the build can verify a lossless round trip. SND, TYREND.ANM, and all 34",
    "作為可重建來源。": "as reproducible source inputs.",
    "Halloween Ramble 隨完整 41 首 PC 曲目保留；": "Halloween Ramble remains as part of the complete 41-song PC catalog;",
    "則承接 可驗證的 Christmas 模式。這些資料都不能在 payload 精簡時誤刪。": "carry the verified Christmas mode. None of these assets may be removed by payload deduplication.",
    ...
    /* Lava / water wave pressure */
    "Lava／Water 波紋壓力控制": "Lava/water wave pressure control",
    "用 source-derived 空間低通與負載遲滯，保留正常波紋並消除極限連射橫帶。": "Use source-derived spatial low-pass filtering and load hysteresis to preserve normal waves while eliminating horizontal bands under maximum fire.",
    "同時保留完整彈幕與正常波紋，讓極限負載不再放大成水平橫帶。": "Preserve both the full projectile field and normal waves without letting extreme load amplify into horizontal bands.",
    "TyrianGbaPoc 如何找出 PENTIUM lava／water 連射橫帶的根因，並以 source-derived 空間低通與負載遲滯保留正常波紋、穩定極限場景。": "How TyrianGbaPoc traced PENTIUM lava/water firing bands to their root cause and used source-derived spatial filtering plus load hysteresis to preserve normal waves and stabilize extreme scenes.",
    "Lava／Water 波紋壓力控制 — TyrianGbaPoc 技術研究": "Lava/water wave pressure control — TyrianGbaPoc research",
    "波紋留下，橫帶消失。": "Keep the waves; lose the bands.",
    "PENTIUM Detail 已能在 GBA 呈現 Tyrian 的 lava／water 色相與波動； 但極限武器連射會把局部濾鏡近似放大成全畫面橫帶。這次改進沒有刪掉 子彈或放慢遊戲，而是讓顯示品質依真實 presentation 壓力平滑調整。": "PENTIUM Detail already brings Tyrian's lava/water color and motion to the GBA. Under maximum weapon fire, however, the local-filter approximation expanded into full-screen horizontal bands. This change neither removes projectiles nor slows the game; it smoothly adjusts display quality according to real presentation pressure.",
    "LAVA EXIT v71 與 v72 同一壓力測試點畫面比較": "LAVA EXIT v71 and v72 at the same stress-test point",
    "v71 修正前": "v71 before",
    "固定整列位移": "Fixed whole-row offset",
    "v71 PENTIUM LAVA EXIT 極限連射畫面，背景出現密集水平橫帶": "v71 PENTIUM LAVA EXIT under maximum fire, with dense horizontal bands across the background",
    "局部 waver 被套到整條 scanline，跳幀後產生高對比橫帶。": "A local waver was applied to an entire scanline, producing high-contrast bands after skipped presentations.",
    "v72 修正後": "v72 after",
    "平滑＋負載遲滯": "Smoothing + load hysteresis",
    "v72 PENTIUM LAVA EXIT 相同極限連射畫面，背景水平橫帶已消失": "v72 PENTIUM LAVA EXIT under the same maximum fire, with the background bands removed",
    "持續極端壓力只暫停幾何位移，lava 色相與完整戰鬥仍保留。": "Sustained extreme pressure pauses only geometric displacement; lava color and the complete battle remain intact.",
    "同一輸入、同一邏輯負載、240×160 原始截圖": "Same input, same logical load, native 240×160 captures",
    "這不是用模糊濾鏡遮住問題。修正前後仍生成 21,794 發玩家射擊、 最高 81 顆 active 彈幕並用滿 128 OAM；敵人、碰撞、RNG、音效與 關卡時間完全不變。": "This is not a blur hiding the problem. Both builds still generate 21,794 player shots, reach 81 active player projectiles, and fill all 128 OAM entries. Enemies, collision, RNG, audio, and stage timing are unchanged.",
    "先分清楚：硬體超載，還是程式破壞畫面？": "First separate hardware overload from corrupted presentation",
    "射擊與不射擊的受控 A/B 給出了清楚界線。不射擊跑到相同地圖位置， 5,908 個 display frames 只有 1 次 missed VBlank；極限連射則有 1,254 次。連射時 logic 平均 126,343 cycles、完整 render 平均 187,921 cycles，兩者相加已超過單一 LCD frame 的 280,896-cycle 預算。": "A controlled firing/no-firing A/B test drew a clear boundary. Reaching the same map position without firing produced only one missed VBlank in 5,908 display frames; maximum fire produced 1,254. Under fire, logic averaged 126,343 cycles and a complete render averaged 187,921 cycles, exceeding the 280,896-cycle budget of one LCD frame when combined.",
    "不射擊 missed VBlank": "Missed VBlank without firing",
    "修正前極限連射 missed VBlank": "Missed VBlank under maximum fire before the fix",
    "DMA0 每個 HBlank 只搬四個 halfword；DMA1/2 由 Maxmod 使用，DMA3 負責一般 VBlank copy。逐項檢查 mGBA raster 行為、雙緩衝 table、 OAM 與 commit 路徑後，沒有 DMA 越界或半張 scene 被提交。正式 drop-frame scheduler 一直在重複上一張完整畫面，再提交下一張完整畫面。": "DMA0 moves only four halfwords per HBlank; Maxmod owns DMA1/2, while DMA3 handles ordinary VBlank copies. Inspection of mGBA raster behavior, double-buffered tables, OAM, and the commit path found no DMA overrun or partially committed scene. The production drop-frame scheduler consistently repeats one complete scene before committing the next complete scene.",
    "因此高 drop-frame 比例是把問題放大的條件，不是橫帶本身的根因。 單純少畫幾張 scene 無法修復錯誤的整列空間近似；它只會降低問題出現 的頻率，同時犧牲本來就有限的 presentation 更新率。": "A high drop-frame rate is therefore the condition that amplifies the problem, not the root cause of the bands. Rendering fewer scenes cannot repair an incorrect whole-row spatial approximation; it only reduces how often the artifact appears while sacrificing an already limited presentation rate.",
    "真正缺口在 PC 濾鏡與 Mode 0 的尺度不同": "The real mismatch is the scale of the PC filter versus Mode 0",
    "OpenTyrian 的 lava_filter 與 water_filter 每八個水平像素計算一次 waver，只改變局部 framebuffer 樣本和鄰列色彩平均。舊 GBA adapter 因為 Mode 0 text BG 無法每八個像素安全改 scroll，只取一個樣本， 再用 HBlank DMA 把它套到整條 BG0／BG1 scanline。": "OpenTyrian's lava_filter and water_filter calculate a waver for every eight horizontal pixels, changing only local framebuffer samples and neighboring-row color averages. Because a Mode 0 text BG cannot safely change scroll every eight pixels, the old GBA adapter took one sample and applied it to an entire BG0/BG1 scanline through HBlank DMA.",
    "正常負載時，這個近似看起來像廉價但有效的水波。極限負載讓相鄰提交 scene 跨過更多 source ticks，高對比 lava 紋理便把一列一列的位移差 顯示成醒目橫帶。硬體超載是真的，但橫帶不是非接受不可的硬體宿命。": "At ordinary load, this approximation looks like an inexpensive but effective wave. Extreme load makes adjacent committed scenes span more source ticks, and high-contrast lava textures expose row-to-row displacement differences as conspicuous bands. The hardware overload is real, but the bands are not an unavoidable hardware fate.",
    "把局部 source 行為翻成適合整列硬體的 profile": "Translate local source behavior into a profile for whole-row hardware",
    "v72 仍依 OpenTyrian 原公式計算每個八像素 block 的 waver，接著對 GBA 實際 240-pixel crop 做可見像素加權平均，並套用五條 scanline 的空間 低通。左右只露出四個像素的 edge blocks 依四個像素計分，完整 blocks 則依八個像素計分，因此不是任選一個畫面中心樣本。": "v72 still computes the waver for each eight-pixel block with OpenTyrian's original formula. It then takes a visible-pixel weighted average over the GBA's actual 240-pixel crop and applies a five-scanline spatial low-pass. Edge blocks exposing only four pixels receive a weight of four, while complete blocks receive eight, rather than choosing an arbitrary center-screen sample.",
    "原公式帶有正向 DC bias；在 PC，它改變的是色彩取樣位置，不是攝影機 整體平移。GBA profile 因而移除這個 DC 分量，只保留真正的波動部分。 Lava／water 色相仍由既有 palette adapter 呈現。": "The original formula carries a positive DC bias. On PC it shifts color sampling, not the whole camera. The GBA profile therefore removes that DC component and retains only actual wave motion. The existing palette adapter continues to provide lava/water color.",
    "依 source 公式建立每個八像素 block 的位移。": "Build each eight-pixel block displacement from the source formula.",
    "對 240-pixel 可見 crop 做精確權重平均。": "Compute an exact weighted average over the visible 240-pixel crop.",
    "用五列空間低通降低整列 scroll 的量化階梯。": "Use a five-row spatial low-pass to reduce quantization steps in whole-row scroll.",
    "移除只屬於色彩取樣的 DC 平移分量。": "Remove the DC translation component that belongs only to color sampling.",
    "在 IRQ 與 Maxmod 啟動前建立 lava／water 兩份固定 profile。": "Build fixed lava and water profiles before IRQs and Maxmod start.",
    "負載遲滯：正常保留，極端才退讓": "Load hysteresis: preserve normal detail and yield only at extremes",
    "Runtime 維護 0 到 24 的 presentation pressure score。Render 被 deadline scheduler 延後、積欠超過一個 source tick，或完整 render 超過 220,000 cycles 時累加；乾淨完成的 scene 讓分數緩慢下降。進入與離開門檻不同， 避免效果在臨界點每幀開關。": "Runtime maintains a presentation-pressure score from 0 to 24. It rises when the deadline scheduler defers a render, debt exceeds one source tick, or a complete render exceeds 220,000 cycles; cleanly completed scenes lower it gradually. Different entry and exit thresholds prevent the effect from toggling every frame near the boundary.",
    "正常：完整平滑波紋": "Normal: full smoothed waves",
    "忙碌：降低幾何幅度": "Busy: reduced geometric amplitude",
    "24/24：暫停幾何位移": "24/24: pause geometric displacement",
    "強度以 Q8 小步下降與更慢速度恢復。即使到 0，停掉的也只有 scanline 幾何位移；lava／water palette、背景捲動與所有 gameplay 仍繼續。 一般不射擊測試最大壓力為 20，不會觸發 24/24 的 severe gate。": "Intensity falls in small Q8 steps and recovers more slowly. Even at zero, only geometric scanline displacement stops; the lava/water palette, background scrolling, and all gameplay continue. The ordinary no-fire test peaks at pressure 20 and never triggers the severe 24/24 gate.",
    "結果：畫面更穩，成本也沒有轉嫁": "Result: a steadier image without shifting the cost elsewhere",
    "兩份固定 profile 改在 IRQ 與音效啟動前預算，首次遇到 lava／water 不再製造長影格。完整強度與零強度另有 fast path，避免每條 scanline 做不必要乘法；因此修正不是以更多漏幀換取較乾淨的截圖。": "Both fixed profiles are now computed before IRQs and audio start, so the first lava/water encounter no longer creates a long frame. Full- and zero-strength fast paths avoid unnecessary multiplication on every scanline, so the cleaner image is not bought with additional missed frames.",
    "完整 render 平均 cycles": "Average cycles for a complete render",
    "玩家射擊生成": "Player shots generated",
    "最大 active 玩家彈幕": "Maximum active player projectiles",
    "最大 OAM": "Maximum OAM",
    "v73：只在 Wave 壓力中分離 Logic 與 Render": "v73: separate logic and render only under wave pressure",
    "v73 再把動態調節限制在來源真正啟用 lava／water 的期間。偵測到持續 deadline miss 後，有跑 source logic 的 LCD frame 優先保留上一張完整 scene，沒有跑 logic 的 frame 才建構最新 OAM、tile cache 與 VRAM transaction。Gameplay、RNG、碰撞與音訊 tick 都沒有省略。": "v73 further limits adaptive scheduling to periods where the source actually enables lava or water. After sustained deadline misses, LCD frames that execute source logic keep the last complete scene, while logic-idle frames build the newest OAM, tile-cache, and VRAM transaction. Gameplay, RNG, collision, and audio ticks are never omitted.",
    "極限連射 missed VBlank": "Missed VBlank under maximum fire",
    "安全強制 render": "Safety-forced renders",
    "同一路線不開火仍只有 1 次 missed VBlank，adaptive entry 為 0；沒有 wave 的 Episode 1 短測 scope attempt 也是 0。這不是全域降幀，而是 在 GBA 已有明確超載證據時，省略永遠不會被顯示的中間 scene 建構。 剩餘 685 次來自單一完整 render 或碰撞 phase 本身仍可能超過一個 LCD frame，因此不能誠稱硬體上限已完全消失。": "The same route without firing still records only one missed VBlank and zero adaptive entries; a short Episode 1 test without waves also records zero scope attempts. This is not a global frame-rate reduction. It omits intermediate scene construction that would never be displayed, and only after the GBA shows clear overload evidence. The remaining 685 misses occur because an individual complete-render or collision phase can still exceed one LCD frame, so the hardware limit has not honestly disappeared.",
    "v73 自適應派送": "v73 adaptive dispatch",
    "沒有採用的捷徑": "Shortcuts we rejected",
    "沒有把 wave phase 與 held scene 分離；否則凍結的世界會自行游動。": "We did not separate wave phase from the held scene; a frozen world must not keep swimming.",
    "沒有刪除或 cap 玩家 projectile；source 邏輯與畫面壓力保持一致。": "We did not remove or cap player projectiles; source logic and presentation pressure remain aligned.",
    "沒有把遊戲邏輯降成 15 或 30 Hz；fixed timestep 仍按正常節奏前進。": "We did not reduce game logic to 15 or 30 Hz; the fixed timestep continues at the normal pace.",
    "沒有用 nearest tile 或模糊後製掩蓋錯誤。": "We did not hide errors with nearest-tile substitution or post-process blur.",
    "AI 提供假說，telemetry 做最後裁決": "AI proposed hypotheses; telemetry made the final call",
    "本次把 DMA 分工、前後 A/B、OpenTyrian filter、cache telemetry 與 GBA Mode 0 限制交給 Gemini 3.1 Pro 複核。值得採用的是空間低通、 遲滯式幅度與 wave 必須跟 held scene 同步；projectile pinning 與 獨立 wave animation 則因不符合現有 cache 或畫面語意而捨棄。": "We asked Gemini 3.1 Pro to review DMA ownership, controlled A/B results, OpenTyrian's filters, cache telemetry, and GBA Mode 0 constraints. Spatial low-pass filtering, hysteretic amplitude, and keeping waves synchronized with the held scene were useful; projectile pinning and independent wave animation were rejected because they did not fit the existing cache or presentation semantics.",
    "最終判斷依據仍是 source code、硬體資料流、定向 ROM telemetry 與 240×160 實際畫面。AI 幫忙縮小搜尋空間，但不替專案發明新規格。": "The final evidence remains source code, hardware data flow, targeted ROM telemetry, and actual 240×160 output. AI narrows the search space; it does not invent a new specification for the project.",
    "極限武器仍會超過 GBA 單幀 CPU 與 OAM 預算，所以 drop-frame 仍是正式 機制。這次突破不是讓限制消失，而是把不可避免的 presentation 退化 限制在最不傷原版語意的位置。": "The maximum weapon load still exceeds the GBA's per-frame CPU and OAM budgets, so drop-frame remains a production mechanism. This breakthrough does not erase the limit; it confines unavoidable presentation degradation to the place least harmful to the original game's meaning.",
    "A/B 證據": "A/B evidence",
    "尺度缺口": "Scale mismatch",
    "空間 profile": "Spatial profile",
    "負載遲滯": "Load hysteresis",
    "捨棄的捷徑": "Rejected shortcuts",

    /* v90 release and LOW website refresh */
    "正式 LOW／Normal 版本，正常傷害、一般劇情裝備，保留 41 首完整 九聲道 OPL2 音樂；新版依原始 envelope 處理 sustain 與自然衰減尾音。 ROM 為 33,472,076 bytes（31.921 MiB）。": "The formal LOW/Normal build uses normal damage and campaign equipment, retains complete nine-channel OPL2 music for all 41 tracks, and now follows the original envelopes for sustain and natural-decay tails. The ROM is 33,472,076 bytes (31.921 MiB).",
    "下載 v90 LOW ROM": "Download the v90 LOW ROM",
    "版本說明與校驗碼": "Release notes and checksum",
    "集中管理開發用無敵模式、極限武器負荷、 HUD 與選單座標。Detail Level 可選 Low、Normal、High、Pentium、 Custom；Game Speed 提供 Low、Normal。正式發佈預設固定使用": "centralizes development invincibility, maximum-weapon stress mode, HUD positions, and menu coordinates. Detail Level supports Low, Normal, High, Pentium, and Custom; Game Speed supports Low and Normal. Formal releases use",
    "，其他等級保留給研究與比較。": "; the other profiles remain available for research and comparison.",
    "目前已打通開頭、設定選單、存讀檔、Game Menu、Upgrade Ship、 Next Level、Demo、JukeBox、關卡、死亡與破關統計等主要流程；四章 62 個 playable sections 都納入資料回歸，少見路線與裝備組合持續 QA。": "The intro, setup menus, save/load, Game Menu, Upgrade Ship, Next Level, Demo, JukeBox, gameplay, death, and end-of-level statistics are connected. All 62 playable sections across four episodes participate in data regression, while uncommon routes and equipment combinations continue through QA.",
    "41 首曲目完整保留九個 OPL2 source channel；關卡歌曲、爆炸與打擊音效、Boss／Data Cube 語音仍依原版 ID 播放。": "All 41 songs retain all nine OPL2 source channels. Stage music, explosions, hit effects, and Boss/Data Cube voices still play from source IDs.",
    "128 個 OAM 必須分配，而非搶先者全拿": "Allocate 128 OAM entries instead of rewarding first arrival",
    "由 owning pool 與 Boss manifest 確定物件身分，先保障玩家、Boss、敵人與玩法子彈；爆炸以公平輪替使用餘額，cache admission 也沿用同一份 presentation snapshot。": "Owning pools and the Boss manifest establish object identity. Players, bosses, enemies, and gameplay projectiles are protected first; explosions rotate fairly through the remainder, and cache admission shares the same presentation snapshot.",
    "OAM 與圖形快取的可靠資源排程": "Reliable OAM and graphics-cache scheduling",
    "確定分類、Boss 預先身分、玩法物件優先與低優先特效公平輪替。": "Deterministic classification, early Boss identity, gameplay-first admission, and fair rotation of lower-priority effects.",
    "v90 正式版採 LOW Detail、Normal Speed、正常傷害與一般劇情裝備； 41 首九聲道音樂改用原始 OPL2 patch 的 adaptive sample，並修正 長音截斷。ROM 以 GitHub Release 資產提供，不存進原始碼 Git； 也可以直接從獨立建置環境自行產生。": "The formal v90 build uses LOW Detail, Normal Speed, normal damage, and regular campaign equipment. All 41 nine-channel tracks now use adaptive samples derived from their original OPL2 patches, with truncated note tails corrected. The ROM is published as a GitHub Release asset rather than stored in source Git, and can also be rebuilt from the self-contained environment.",

    /* v90 audio fidelity refresh */
    "PC 原版的 OPL2 音樂不能原封不動交給 GBA 播放。本專案將事件、音色與 聲道關係轉成 Maxmod 可負擔的 tracker／PCM 表示，完整保留九個 OPL2 source channel，並以離線量測修正聲道響度、鼓聲瞬態、有限提示曲與場景切換。": "The PC version's OPL2 music cannot be handed directly to the GBA. This project turns its events, timbres, and channel relationships into a tracker/PCM representation Maxmod can sustain, retaining all nine OPL2 source channels while offline measurement corrects loudness, percussion transients, finite cues, and scene transitions.",
    "與原始 song ID 決定流程； build 端使用 41 首 TYM 表示保存 LDS／OPL2 pattern、patch、note、 channel volume 與 carrier total-level，再轉成 Maxmod 的 IT modules； 41 首中實際使用第九聲道的 26 首不再刪除最低能量聲部。 音效則直接由": "and source song IDs still drive the flow. At build time, 41 TYM representations preserve LDS/OPL2 patterns, patches, notes, channel volumes, and carrier total levels before conversion to Maxmod IT modules. The 26 songs that use a ninth channel no longer lose their quietest part. Sound effects come directly from",
    "程序鼓聲以 GBA mixer 原生 15,768 Hz 產生，並依類型使用不同 sample length，不再共用同一段 46 ms noise。 每個 one-shot 使用 1.5 ms smoothstep attack 與 5 ms release，並只移除 極低頻 DC；沒有對全部曲目強套 120 Hz high-pass 或 8 kHz shelf， 避免把不同 Tyrian 音色 EQ 成同一種聲音。": "Procedural percussion is generated at the GBA mixer's native 15,768 Hz with type-specific sample lengths instead of one shared 46 ms noise burst. Each one-shot uses a 1.5 ms smoothstep attack and 5 ms release while removing only very-low-frequency DC; no blanket 120 Hz high-pass or 8 kHz shelf forces unlike Tyrian timbres into the same EQ.",
    "Build 會輸出逐曲、逐 source 的 gain、RMS error、peak ceiling、 clipping 與 track mapping 報告；回歸另驗證 41 首／334 個 OPL sources、 三首 finite cues、18 個 mixer slots、場景淡出／靜音／淡入步數、自然 停止次數與所有 source SFX ID。人耳 A/B 仍不可取代，但每次修改不必 再從零猜測「哪個聲道變大」。": "The build reports per-track and per-source gain, RMS error, peak ceiling, clipping, and track mapping. Regression also verifies 41 songs and 334 OPL sources, three finite cues, 18 mixer slots, scene fade/silence/fade-in steps, natural-stop counts, and every source SFX ID. Human A/B remains essential, but each change no longer starts by guessing which channel became louder.",
    "下一篇：OAM scheduling": "Next: OAM scheduling",

    /* Current research facts */
    "八條已開放雙向路徑各做 120 次，共 960 次。原始完整矩陣以 High／Normal 驗證，結果為 0 missed VBlank、0 runtime SHP、 0 runtime Sprite2、music active；最大單 phase 118,465 cycles，低於 180,000-cycle 回歸門檻。v90 LOW 正式版沿用相同、與 detail 無關的管線。": "Eight enabled bidirectional paths run 120 times each, for 960 transitions. The original full matrix used High/Normal and recorded zero missed VBlanks, zero runtime SHP decodes, zero runtime Sprite2 decodes, and continuous music. The largest phase cost 118,465 cycles, below the 180,000-cycle regression limit. The v90 LOW release uses the same detail-independent pipeline.",
    "TyrianGbaPoc 技術研究：GBA 受限色盤、多圖層、Sprite2／OAM 資源排程、ROMFS、固定時序、音訊與靜態選單最佳化。": "TyrianGbaPoc technical research: constrained GBA palettes, layers, Sprite2/OAM resource scheduling, ROMFS, fixed timing, audio, and static-menu optimization.",
    "以確定身分、單一候選快照、Boss 預先 manifest 與公平輪替，讓玩法物件不被低優先特效擠掉。": "Use deterministic identity, one candidate snapshot, an early Boss manifest, and fair rotation so low-priority effects cannot displace gameplay objects.",
    "對照圖保留當時 v57 的相同鏡位，方便呈現演算法前後差異；v90 LOW 正式版仍由同一套全 62 關訓練與雙感知色差 gate 產生背景 palette。": "The comparison keeps the original v57 camera position to show the algorithmic difference. The v90 LOW release still generates background palettes through the same 62-stage training set and dual perceptual-error gates.",
    "原始 Episode 2 High／Normal A/B 中，25-row 柔性鏡頭與固定 crop 都是 41 missed VBlanks；也就是增加視野活動範圍後，背景換列成本仍回到 固定鏡頭基線。v90 LOW 正式版沿用相同 camera 與 ownership band； dynamic frame drop 保存的 held window 也使用 camera-adjusted scroll，延後 presentation 時不會提前釋放仍在畫面上的 row。": "In the original Episode 2 High/Normal A/B, both the 25-row soft camera and fixed crop recorded 41 missed VBlanks. The broader viewing range therefore returns background-row cost to the fixed-camera baseline. The v90 LOW release uses the same camera and ownership band; held windows in dynamic frame drop also use camera-adjusted scroll so deferred presentation never releases rows that remain visible.",
    "ROMFS 不以「目前沒有呼叫」為刪除條件。只有另一份 cartridge payload 已完整承接，且 build 能做無損 round-trip 時，才排除重複 原始流。目前 SND、Christmas SND/SHP、TYREND.ANM 與 34 份": "ROMFS does not delete data merely because it is not currently called. A duplicate source stream is omitted only when another cartridge payload completely replaces it and the build can prove a lossless round trip. Current replacements cover SND, Christmas SND/SHP, TYREND.ANM, and 34",
    "共省下 5,420,450 bytes 的重複 payload；原始檔仍保留在": "together saving 5,420,450 bytes of duplicate payload; source files remain in",
    "自 v89 起，專案以全 62 關 build-time Boss manifest、runtime instance generation 與 4bpp compact cache 處理血條出現前及移動分離的超大型 Boss。這些 presentation ownership 規則整理於": "Since v89, the project has used a build-time Boss manifest for all 62 stages, runtime instance generations, and a 4bpp compact cache for very large bosses before their health bar appears and while components separate in motion. These presentation-ownership rules are documented in",
    "OAM 與圖形快取資源排程": "OAM and graphics-cache scheduling",
    "總 missed VBlank 會分成 gameplay、stats、game-over、frontend transition 與其他 frontend。這能分辨「Boss 計算真的重」和「選單誤做 runtime SHP 解碼」。v90 正式版使用 LOW／Normal；完整第一關 gameplay gate 為 2%，較密集的 Episode 2 stock route 為 2.5%，而 frontend、stats、 game-over 與 transition deterministic misses 仍必須為 0。": "Total missed VBlanks are split among gameplay, stats, game over, frontend transitions, and other frontend work. This distinguishes a genuinely expensive boss from accidental runtime SHP decoding in a menu. v90 uses LOW/Normal: the complete first-stage gameplay gate is 2%, the denser stock Episode 2 route allows 2.5%, and deterministic frontend, stats, game-over, and transition misses must remain zero.",
    "Episode 1 campaign、Episode 2／3／4 routes 與存檔續關案例，驗證 script 轉場、Boss exit 與不同資產 bank。": "Episode 1 campaigns, Episode 2/3/4 routes, and save-resume cases verify script transitions, boss exits, and different asset banks.",
    "Missed VBlank 分為 gameplay、stats、game over、transition 與 frontend other。v90 LOW 正式版的發佈驗證另固定檢查：": "Missed VBlanks are split among gameplay, stats, game over, transitions, and other frontend work. The v90 LOW release validation also fixes these checks:",

    /* OAM and cache scheduling */
    "TyrianGbaPoc 如何用確定物件身分、Boss manifest、單一候選快照與公平輪替，在 GBA 的 128 OAM 與有限 OBJ VRAM 內保住玩法物件。": "How TyrianGbaPoc uses deterministic identity, a Boss manifest, one candidate snapshot, and fair rotation to preserve gameplay objects within 128 GBA OAM entries and limited OBJ VRAM.",
    "OAM 與圖形快取的可靠資源排程 — TyrianGbaPoc 技術研究": "Reliable OAM and graphics-cache scheduling — TyrianGbaPoc research",
    "先確定「它是誰」，再分配 128 個 OAM。": "Establish what an object is before allocating 128 OAM entries.",
    "高負荷畫面不能讓先被迴圈走到的爆炸拿走所有 OBJ。可靠的排程必須讓 物件分類、OAM admission 與圖形快取共享同一份事實，先保住會影響玩法 的內容，再讓裝飾效果公平使用剩餘硬體。": "A high-load scene cannot let the first explosions traversed claim every OBJ. Reliable scheduling makes object classification, OAM admission, and graphics caches share one source of truth, protecting gameplay content before cosmetic effects fairly use the remainder.",
    "真正的失效不只一種": "There is more than one failure mode",
    "GBA 全畫面最多提交 128 個 OBJ，同一掃描線還有更小的像素預算； Tyrian 的多部件 Boss、玩家與敵方彈幕、Sidekick、獎賞及爆炸可能同時 超過兩者。更隱蔽的是 OBJ VRAM 快取：敵人仍存在、仍可碰撞，卻可能 因低優先動畫先占滿圖形格而成為透明物件。": "The GBA can submit at most 128 OBJs across a frame and has an even smaller per-scanline pixel budget. Tyrian's multipart bosses, friendly and enemy projectiles, sidekicks, rewards, and explosions can exceed both. OBJ VRAM caching creates a subtler failure: an enemy can remain alive and collidable yet become transparent because low-priority animation occupied its graphics slots.",
    "單純把爆炸排到 OAM 尾端仍不夠。如果它已先污染共享圖形快取， 後面的 Boss 即使取得 OAM，也沒有正確 tile 可以顯示。": "Putting explosions at the end of OAM is not enough. If they polluted the shared graphics cache first, a later boss can receive OAM but still have no correct tile to display.",
    "物件身分不能靠座標或圖號猜": "Object identity cannot be guessed from coordinates or graphics IDs",
    "玩家、玩家子彈、敵方子彈與效果分屬固定 owning pool，可直接取得確定 類別。Boss components 與一般敵人共用 enemy pool，則由 build 工具掃描 四章全部 LVL 的 Event 79、link alias、spawn cohort 與 group-control event，產生通用 Boss identity manifest。": "The player, friendly projectiles, enemy projectiles, and effects belong to fixed owning pools and therefore have deterministic classes. Boss components share the enemy pool with ordinary enemies, so a build tool scans every LVL across four episodes, following Event 79, link aliases, spawn cohorts, and group-control events to generate a general Boss identity manifest.",
    "Runtime 在 spawn 當下以": "At spawn time, runtime binds identity through",
    "綁定身分；零件在 登場動畫中分離或移動，仍是同一個 Boss。slot 釋放後 generation 改變， 新敵人不會繼承舊身分。這也讓血條尚未出現的前置登場階段就能取得 Boss 優先權，而不必為個別關卡寫死 link 清單。": ". Components remain part of the same boss while separating or moving during the entrance animation. Releasing a slot changes its generation, so a new enemy cannot inherit old identity. Boss priority is therefore available before the health bar appears, without hard-coded per-stage link lists.",
    "一張候選快照，貫穿三個決策": "One candidate snapshot drives three decisions",
    "每個 presentation frame 只建立一次候選目錄，預先保存 screen coordinate、 可見性、OAM 成本、draw stage 與 owning class。需求計數、圖形快取預鎖、 OAM admission 與最後 render 都讀同一份 snapshot，不會在四次 pool 掃描間 得到彼此矛盾的分類。": "Each presentation frame builds one candidate catalog containing screen coordinates, visibility, OAM cost, draw stage, and owning class. Demand counting, graphics-cache priming, OAM admission, and final rendering all read the same snapshot instead of deriving contradictory classifications across four pool traversals.",
    "玩家、Boss 結構、一般敵人及主要敵我子彈先取得 OAM 與 cache。": "Players, boss structures, ordinary enemies, and primary friendly and enemy projectiles receive OAM and cache space first.",
    "爆炸、短命效果與陰影只使用剩餘額度。": "Explosions, short-lived effects, and shadows use only the remaining budget.",
    "超額候選從上次停止位置續跑，不讓固定低索引永遠顯示、其他物件永遠消失。": "Excess candidates resume where the previous frame stopped, preventing fixed low indexes from always appearing while other objects vanish forever.",
    "Pending upload 帶 ownership generation；資源被重用後，舊 DMA 工作不能覆寫新 tile。": "Pending uploads carry an ownership generation, so stale DMA work cannot overwrite a new tile after resources are reused.",
    "超過硬體上限時，退化也要可預期": "Degradation must remain predictable beyond the hardware limit",
    "最壞壓力研究曾量到同一場景提出 223 個 OAM 需求。排程仍只提交硬體允許 的 128 個：敵人淘汰為 0、敵人快取預鎖失敗為 0，低優先爆炸承擔主要 裁切；若連結構物本身都超量，結構物 admission 也會跨 presentation frame 輪替。更新、碰撞、傷害與原始 pool 順序完全不變。": "The worst stress study measured 223 OAM requests in one scene. The scheduler still commits only the hardware's 128: enemy culls and enemy-cache prime failures remain zero, while low-priority explosions absorb most cuts. If structural objects themselves exceed capacity, their admission rotates across presentation frames. Updates, collision, damage, and source pool order remain unchanged.",
    "候選快照、合併 prime、飽和需求計數、稀疏／密集混合掃描與無 modulo 的兩段輪替，讓同一壓力測試 render cycles 再下降 3.86%，完成一張畫面的 平均成本下降 6.01%。這類分支繁多、資料導向的排程保留在最佳化 C； 實測 ARM 化 bit-scan 反而較慢，沒有為「用了組語」犧牲 IWRAM。": "The candidate snapshot, merged priming, saturated demand counting, hybrid sparse/dense scans, and modulo-free two-range rotation reduce render cycles by another 3.86% in the same stress test and lower average completed-frame cost by 6.01%. This branch-heavy, data-oriented scheduler stays in optimized C: measured ARM bit scans were slower, so IWRAM was not sacrificed merely to claim assembly use.",
    "Cache ownership 是同一套規則的另一半": "Cache ownership is the other half of the same rule",
    "Boss compact cache 將 32×32 Sprite2 從 1 KiB 8bpp 格轉成 512-byte 4bpp 畫布，並以關卡 palette 訓練兩組可見色，讓大型 Boss 的 40 個以上部件 可同時存在。一般 L1/L2、projectile cache、split Sidekick 與 compact cache 都驗證 generation 及 layout；例如 fragmented 32×32 Sidekick 必須 以兩個 32×16 OBJ 拼接，不能把相鄰 cache 的 tile 當成下半部。": "The Boss compact cache converts 32x32 Sprite2 graphics from 1 KiB 8bpp slots into 512-byte 4bpp canvases, training two visible-color palettes for the level so more than 40 large boss components can coexist. Ordinary L1/L2, projectile, split-sidekick, and compact caches all validate generation and layout; a fragmented 32x32 sidekick, for example, must join two 32x16 OBJs rather than treating tiles from an adjacent cache as its lower half.",
    "v90 LOW 正式管線中的多部件 Boss、玩家彈幕與爆炸壓力畫面": "Multipart boss, player projectiles, and explosion pressure in the formal v90 LOW pipeline",
    "最終原則": "Final rule",
    "128 OAM 是硬限制，但「哪個物件先消失」是軟體設計。當分類來自資料 ownership、Boss 身分在 build 與 runtime 都可驗證、OAM 與 cache 共用 admission snapshot，超載就能退化成可控的低優先閃爍，而不是偶發的 透明敵人、Boss 缺件或玩家武器被錯圖污染。": "The 128-entry OAM limit is hardware, but which object disappears first is software design. When classification comes from data ownership, Boss identity is verifiable at build time and runtime, and OAM plus caches share one admission snapshot, overload becomes controlled low-priority flicker instead of intermittent transparent enemies, missing boss parts, or corrupted player weapons.",
    "失效類型": "Failure modes",
    "確定物件身分": "Deterministic object identity",
    "候選快照": "Candidate snapshot",
    "超載退化": "Overload degradation",

    /* Adaptive presentation scheduling */
    "Adaptive 呈現派送": "Adaptive presentation dispatch",
    "持續超載時將畫面穩定降至不低於約 17.4 FPS，遊戲邏輯與音訊仍保持原速。": "Under sustained overload, presentation settles no lower than about 17.4 FPS while game logic and audio remain at full speed.",
    "TyrianGbaPoc 如何以 Drop-frame 與 Adaptive presentation dispatch，在不降低遊戲邏輯及音訊時鐘的前提下穩定處理 GBA 持續超載。": "How TyrianGbaPoc combines frame dropping and adaptive presentation dispatch to handle sustained GBA overload without slowing the simulation or audio clock.",
    "Adaptive 呈現派送與 Drop-frame — TyrianGbaPoc 技術研究": "Adaptive presentation dispatch and frame dropping — TyrianGbaPoc research",
    "讓過載變成穩定節奏，不讓遊戲世界變慢。": "Turn overload into a stable cadence without slowing the game world.",
    "偶發漏畫與持續超載是兩種不同問題。Drop-frame 保護單一 LCD frame 的提交期限；Adaptive presentation dispatch 則在量測到長期壓力後，主動降低完整場景的建構頻率，同時維持遊戲邏輯、音訊與關卡流程的原始節奏。": "An occasional missed presentation and sustained overload are different problems. Frame dropping protects an individual LCD frame's commit deadline; after measuring persistent pressure, adaptive presentation dispatch deliberately reduces complete-scene construction frequency while preserving the original cadence of game logic, audio, and level progression.",
    "兩套互補、不能混為一談的機制": "Two complementary mechanisms that must not be confused",
    "是逐 LCD frame 的 deadline scheduler。若估算新場景無法在安全期限前完成，就保留上一張已完整提交的 OAM、VRAM 與背景狀態，避免把半套畫面交給顯示硬體。": "is a per-LCD-frame deadline scheduler. If a new scene cannot finish before the safe deadline, it retains the last completely committed OAM, VRAM, and background state instead of handing partial presentation data to the display hardware.",
    "是建立在 Drop-frame 上的壓力狀態機。它觀察連續 deadline 壓力與真正的 missed VBlank；確認不是一次冷快取尖峰後，才把完整場景建構鎖到較低且規律的 cadence，避免不穩定地連續超時。": "is a pressure state machine built on frame dropping. It observes sustained deadline pressure and actual missed VBlanks, then locks complete-scene construction to a lower, regular cadence only after ruling out a one-off cold-cache spike.",
    "這裡降低的是 presentation FPS，不是內部遊戲邏輯 FPS。兩者若被混稱為「降內部 FPS」，很容易誤以為敵人、碰撞或音樂也被放慢。": "What changes here is presentation FPS, not the internal simulation rate. Calling both of them a lower internal FPS would wrongly suggest that enemies, collision, or music also slow down.",
    "哪些工作永遠不能被省略": "Work that may never be skipped",
    "Adaptive 只捨棄已被較新狀態取代、尚未送出的中間畫面。下列 authoritative state 仍按原順序執行：": "Adaptive discards only unsubmitted intermediate presentations already superseded by newer state. The following authoritative work still runs in source order:",
    "玩家輸入、移動、武器 cooldown 與能量；": "player input, movement, weapon cooldowns, and energy;",
    "敵人事件、Boss 流程、碰撞、傷害與掉落；": "enemy events, boss flow, collision, damage, and drops;",
    "關卡位置、script 分支與 RNG 呼叫順序；": "level position, script branches, and RNG call order;",
    "每個實體 VBlank 的 Maxmod 音樂與音效服務。": "Maxmod music and sound-effect service on every physical VBlank.",
    "因此畫面在壓力下可能維持上一個完整 frame，但下一張出現時會直接反映正確時間點，不會因少畫一張而讓關卡慢動作或改變戰鬥結果。": "The display may therefore hold the last complete frame under pressure, but the next presentation immediately reflects the correct point in time. Omitting one presentation never turns the level into slow motion or changes the combat result.",
    "三段壓力狀態與 17.4 FPS 下限": "Three pressure states and the 17.4 FPS floor",
    "預設；安全預算足夠就呈現": "Default; present whenever the safe budget permits",
    "每兩個 source tick 一張": "One complete scene per two source ticks",
    "同為兩 tick；只保留高壓分類": "Also two ticks; retained only as a high-pressure class",
    "Tyrian 的 Normal speed 約為 34.8 logic tick/s。正式設定將一張刻意輸出的場景限制為最多代表兩個 source ticks，因此 Medium／Severe 的目標下限約為 17.4 FPS。早期 Severe 曾允許三 tick、約 11.6 FPS；實機觀感過於不連續，現行程式已從型別與編譯檢查上禁止回到該設定。": "Tyrian's Normal speed is about 34.8 logic ticks per second. Release settings limit an intentionally presented scene to at most two source ticks, giving Medium and Severe a target floor near 17.4 FPS. An early Severe state allowed three ticks, or about 11.6 FPS; hardware testing looked too discontinuous, so current type and compile-time checks prohibit returning to that setting.",
    "只對持續壓力反應，不因單一尖峰抖動": "React to sustained pressure, not a single spike",
    "一般 gameplay 必須累積至少 4 點 deadline 壓力及 2 次 missed VBlank 才進入 Adaptive；壓力達 8 或 missed VBlank 達 4 時標記為 Severe。lava／water 場景因波紋還占用顯示時序，在確認 2 點壓力及 1 次 missed VBlank 後可提早進入 Severe，但仍受相同 17.4 FPS 下限約束。": "Ordinary gameplay must accumulate at least four deadline-pressure points and two missed VBlanks before entering Adaptive. Eight pressure points or four misses mark Severe. Because lava and water waves also consume display timing, those scenes may enter Severe after two pressure points and one missed VBlank, but the same 17.4 FPS floor still applies.",
    "連續 16 張完整 render 都低於 150,000 cycles 才退出，避免 cadence 來回跳動；": "exit only after 16 consecutive complete renders below 150,000 cycles, preventing cadence oscillation;",
    "最多保留兩個 pending logic ticks，超過就強制產生新場景；": "retain at most two pending logic ticks before forcing a new scene;",
    "背景 ring row、關卡狀態轉換及 freshness 條件永遠可以強制 render；": "background ring rows, level-state transitions, and freshness constraints may always force a render;",
    "一般 Boss、爆炸與複雜武器使用同一套全域量測，不靠 per-level 特例。": "ordinary bosses, explosions, and complex weapons use the same global measurements rather than per-level exceptions.",
    "歷史 A/B：一般非 wave 關卡也能受益": "Historical A/B: ordinary non-wave stages benefit too",
    "Episode 2 Section 1 的 600-VBlank 壓力測試，在遊戲邏輯更新、RNG、玩家子彈數與音訊結果完全相同時，加入 Adaptive 後 missed VBlank 由 165 降至 93，減少 43.64%；平均 loop work 由 224,781.42 降至 186,695.33 cycles，減少 16.94%，兩版 audio frame loss 都是 0。": "In a 600-VBlank Episode 2 Section 1 stress test with identical logic updates, RNG, player-shot counts, and audio results, Adaptive reduced missed VBlanks from 165 to 93, a 43.64% reduction. Average loop work fell from 224,781.42 to 186,695.33 cycles, or 16.94%, while both builds recorded zero audio-frame loss.",
    "這組數據來自能降至三 tick 的歷史基準，用來證明 Adaptive 不只對 lava／water 特例有效。現行 v90 Release 已將流暢度優先權提高，Medium 與 Severe 都封頂為兩 tick，因此不應把歷史 complete-render 數量直接當成目前版本預期值。": "These figures come from a historical baseline that could fall to three ticks; they establish that Adaptive works beyond lava and water special cases. The current v90 release gives smoothness higher priority and caps both Medium and Severe at two ticks, so historical complete-render counts must not be treated as current expectations.",
    "安全網不是免除最佳化的魔法": "A safety net does not eliminate optimization",
    "GBA 每個 LCD frame 約有 280,896 CPU cycles。若單一不可切割的 render、解碼或 cache miss 本身已超過這個預算，Drop-frame 與 Adaptive 只能避免它再和其他昂貴 phase 疊在一起，不能縮短該工作本身。這類瓶頸仍要以 build-time 預處理、DMA、cache、IWRAM／ARM leaf kernel 或演算法改善解決。": "A GBA LCD frame provides about 280,896 CPU cycles. If one indivisible render, decode, or cache miss exceeds that budget by itself, frame dropping and Adaptive can only keep it from overlapping another expensive phase; they cannot shorten the work itself. Such bottlenecks still require build-time preprocessing, DMA, caching, IWRAM/ARM leaf kernels, or algorithmic improvements.",
    "正式版因此同時開啟 Dynamic Drop-frame、全域 Adaptive presentation dispatch 與 wave 快速進入策略。關閉 Adaptive 只用於受控 A/B；它不是一般 Release 選項，也不套用到靜態前端選單。": "Release builds therefore enable Dynamic Drop-frame, global adaptive presentation dispatch, and early wave-scene entry together. Disabling Adaptive is reserved for controlled A/B tests; it is not a normal release option and does not apply to static front-end menus.",
    "過載時可以少交付一張已過時的畫面，但不能少算一次會改變遊戲結果的工作。把 authoritative simulation 與可被取代的 presentation 分開，才讓 GBA 在極端火力下維持音樂與正確流程，同時把視覺退化限制為穩定、可量測且有最低下限的呈現頻率。": "Under overload, an obsolete presentation may be omitted, but no work that changes the game result may be skipped. Separating authoritative simulation from replaceable presentation lets the GBA preserve music and correct flow under extreme firepower while constraining visual degradation to a stable, measurable cadence with a firm floor.",
    "上一篇：OAM scheduling": "Previous: OAM scheduling",
    "固定時序基礎": "Fixed-timing fundamentals",
    "TyrianGbaPoc 技術研究：GBA 受限色盤、多圖層、Sprite2／OAM 資源排程、ROMFS、固定時序、Adaptive 呈現、音訊與靜態選單最佳化。": "TyrianGbaPoc technical research: constrained GBA palettes, layers, Sprite2/OAM resource scheduling, ROMFS, fixed timing, adaptive presentation, audio, and static-menu optimization.",
    "Adaptive 呈現派送與 Drop-frame": "Adaptive presentation dispatch and frame dropping",
    "持續超載時穩定降低完整場景頻率，同時保持遊戲邏輯、音訊與關卡流程的真實時間節奏。": "Stabilize complete-scene frequency under sustained overload while game logic, audio, and level progression retain their real-time cadence.",
    "Drop-frame 處理單一 frame 的截止期限；持續超載則交給": "Frame dropping handles one frame's deadline; sustained overload is handled by",
    "，將完整場景鎖到穩定且有下限的 cadence。": ", which locks complete scenes to a stable cadence with a defined floor.",
  });
